...
from src.utils.mh_dist import compute_pixelwise_mahalanobis_distance_slicewise_3d
...
class DDPM_2D(LightningModule):
    ...
    def test_step(self, batch: Any, batch_idx: int):
        ...
        loss_diff = 0
        reco_ensemble = []
        noise_levels = [self.test_timesteps]
        for n in noise_levels:
            timesteps = [n-1] * self.cfg.get('num_mc_samples',10) # timesteps to evaluate
            for idx, t in enumerate(timesteps):
                # generate noise
                noise = gen_noise(self.cfg, input.shape).to(self.device)
                # reconstruct
                loss, reco = self.diffusion(input,cond=features,t=t-1,noise=noise)
                loss_diff += loss
                reco_ensemble.append(reco)
        reco = torch.stack(reco_ensemble) # average over timesteps
        reco_mean = torch.mean(reco,0)
        loss_diff /= len(timesteps)
        # MHD
        mask = data_mask.squeeze().permute(2,0,1) if self.cfg.get('masked_mhd',False) else None # use mask to save resources
        mh_dist = compute_pixelwise_mahalanobis_distance_slicewise_3d(input.squeeze(),reco.squeeze(),mask=mask,regu=self.cfg.get('mh_regu',1e-2))
        # replace nan and inf with 0
        mh_dist[mh_dist != mh_dist] = 0
        # replace inf with 0
        mh_dist[mh_dist == float('inf')] = 0
        # calc sample_wise MHD
        reco = reco_mean
        mh_dist = mh_dist.unsqueeze(1)
        # min max normalization
        mh_dist = (mh_dist - mh_dist.min()) / (mh_dist.max() - mh_dist.min())
        cam = mh_dist 
        ...
        cam = cam.squeeze().permute(1,2,0).cpu() # to HxWxD
        ...
        if cam is not None:
            ...
            cam = gaussian_filter(cam.float(), sigma=1) 
            # to tensor
            cam = torch.tensor(cam,device='cpu')
